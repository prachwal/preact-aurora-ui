...
export const estimateComponentSize = (element: HTMLElement): number => {
...
export const expectPerformance = (metrics: Partial<PerformanceMetrics>) => {
  const results = {
    renderTimeOk:
      !metrics.renderTime || metrics.renderTime <= PERFORMANCE_THRESHOLDS.RENDER_TIME_MS,
    componentSizeOk:
      !metrics.componentSize ||
      metrics.componentSize <= PERFORMANCE_THRESHOLDS.COMPONENT_SIZE_KB * 1024,
    reRenderCountOk:
      !metrics.reRenderCount || metrics.reRenderCount <= PERFORMANCE_THRESHOLDS.MAX_RE_RENDERS,
  };
